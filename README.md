## 👋 Hi, I’m Shreya Mahara

🚀 Aspiring Cloud & Software Engineer | AWS | Python | Java | Docker | Containers    

---

##👩‍💻 About Me  

I'm a Computer Science Engineering student with a strong passion for **Cloud Computing and DevOps**. I enjoy designing, automating, and monitoring scalable systems while  strengthening my fundamentals through hands-on experimentation.

I believe in **learning** by doing, contributing to projects, and sharing my journey publicly to stay consistent and accountable.  

---

## 🛠️ Tech Stack

### 💻 Languages & Frameworks
![Python](https://skillicons.dev/icons?i=python)
![HTML](https://skillicons.dev/icons?i=html)
![CSS](https://skillicons.dev/icons?i=css)
![JavaScript](https://skillicons.dev/icons?i=js)

### ☁️ Cloud & DevOps
![AWS](https://skillicons.dev/icons?i=aws)
![Docker](https://skillicons.dev/icons?i=docker)
![Google Cloud](https://skillicons.dev/icons?i=gcp)

### 📊 Databases & Monitoring
![MySQL](https://skillicons.dev/icons?i=mysql)
![MongoDB](https://skillicons.dev/icons?i=mongodb)   

---

## 📚 Currently Learning

- Python for DevOps & Automation
- AWS Cloud Services (EC2, S3, IAM)
- Linux, Git & GitHub
- Monitoring & system health checks
- Cloud-native best practices

## 📂 Projects & Practice

- 🐍 Python automation scripts
- ⚙️ System monitoring tools
- ☁️ AWS hands-on mini projects
- 🐳 Docker containerization practice 
- 📦 Creating multi-container apps using Docker Compose
- ☸️ Learning Kubernetes basics
- 🔄 CI/CD practice with container workflows

*(More projects coming as I build consistently 🚀)*

---

## 🤝 Open to Collaborate

I'm interested in collaborating on anything related to:

- 💻 Software Engineering & Development projects
- ☁️ Cloud & DevOps projects
- 🐍 Python automation tools
- 🚀 Beginner-friendly open-source contributions

  *"Always excited to learn, build, and grow through real-world development experience."*

---

  ## 🌐 Connect With Me

- 💼 [LinkedIn](https://www.linkedin.com/in/shreya-mahara-5a906b28b/)
- 🏅 [Credly](https://www.credly.com/users/shreya-mahara)
- 📧 shreyamahara17@gmail.com
- 📸 [@_shreyamahara_](https://www.instagram.com/_shreyamahara_?igsh=MTFxbnF6aWhzN2RjMQ==)

  ---

  *“Currently deepening my expertise in AWS, containerization, and workflow automation."*

<!--**shreya172004/shreya172004** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
